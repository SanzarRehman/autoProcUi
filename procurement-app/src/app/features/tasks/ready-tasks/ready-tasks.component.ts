...
/**
 * Component for displaying ready tasks
 * Shows tasks that are available to be claimed by users
 */
@Component({
  selector: 'app-ready-tasks',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  templateUrl: './ready-tasks.component.html',
  styleUrls: ['./ready-tasks.component.scss']
})
export class ReadyTasksComponent implements OnInit {
   ...
   * Fetches ready tasks with pagination
   */
  fetchReadyTasks = async (offset: number, limit: number): Promise<{ rows: Task[]; lastRow: number }> => {
    try {
      this.error = null;

      // Check if we have cached data for the first page (only when no filter is active)
      if (offset === 0 && limit <= 50 && !this.filterCode) {
        const cached = this.taskStatsService.getCachedReadyTasks();
      ...
      if (offset === 0 && !this.filterCode) {
        this.taskStatsService.updateReadyCache(response);
      }
      ...
      this.error = error.message || 'Failed to load ready tasks';
      this.errorHandler.handleError(error, 'Failed to load ready tasks');
      throw error;
    }
  };
