...
/**
 * Component for displaying own tasks
 * Shows tasks that are assigned to the current user
 */
@Component({
  selector: 'app-own-tasks',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
...
export class OwnTasksComponent implements OnInit {
   ...
   * Fetches own tasks with pagination
   */
  fetchOwnTasks = async (offset: number, limit: number): Promise<{ rows: Task[]; lastRow: number }> => {
    try {
      this.error = null;

      // Check if we have cached data for the first page (only when no filter is active)
      if (offset === 0 && limit <= 50 && !this.filterCode) {
        const cached = this.taskStatsService.getCachedOwnTasks();
      ...
      if (offset === 0 && !this.filterCode) {
        this.taskStatsService.updateOwnCache(response);
      }
      ...
      this.error = error.message || 'Failed to load own tasks';
      this.errorHandler.handleError(error, 'Failed to load own tasks');
      throw error;
    }
  };
